...
public class Reproductor {
	...
	/*Se quizo usar hashset pero hashset "creo" que solo me permite comparar las siguientes opciones
	 * 1) this.username == other.usename; entonces el hashcode hace 
	 * 			result = prime * result + ((username == null) ? 0 : username.hashCode());
	 * 
	 * 2) this.mail == other.mail; entonces el hashcode hace 
	 * 		result = prime * result + ((mail == null) ? 0 : mail.hashCode())
	 * 
	 * 3) (this.username == other.username && this.mail == other.mail entonces el hashcode hace lo siguiente:
	 * 		result = prime * result + ((username == null) ? 0 : username.hashCode());
	 * 		result = prime * result + ((mail == null) ? 0 : mail.hashCode())
	 * 		
	 * Pero no se pudo en nuestro caso de regla de negocio si (this.username == other.usename) || (this.mail == other.mail);
	 * tendria que cambiar el hashcode pero no sé como
	 */
		...
		if(user.esValidoUsuario()) {
			if(this.usuarios.size()==0) {
				...
				for(int i=0; i<this.usuarios.size(); i++) {
					if(!this.encontrarUsuarioPorUserName(user.getUsername()) && !this.encontrarUsuarioPorMail(user.getMail())) {
						this.usuarios.add(user);
						exitoso = true;
					}
				}
